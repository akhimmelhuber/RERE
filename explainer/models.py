...
class PolicyNetworkRescal(torch.nn.Module):
    def __init__(self, input_dim=10, hidden_dim=10, num_heads=2):
        super(PolicyNetworkRescal, self).__init__()

        self.conv1 = GATConv(input_dim, hidden_dim, heads=2)
        self.conv2 = GATConv(num_heads * hidden_dim, hidden_dim, heads=2)

        self.stop_action = torch.nn.Parameter(torch.Tensor(1, 2*hidden_dim))
        self.link_pred_source = torch.nn.Parameter(torch.Tensor(2*hidden_dim, 2*hidden_dim))
        self.link_pred_target = torch.nn.Parameter(torch.Tensor(2*hidden_dim, 2*hidden_dim))
        
        self.output = torch.nn.Parameter(torch.Tensor(4*hidden_dim, 1))

        torch.nn.init.xavier_uniform_(self.stop_action)
        torch.nn.init.xavier_uniform_(self.link_pred_source)
        torch.nn.init.xavier_uniform_(self.link_pred_target)

    def forward(self, x, edge_index, source=None):
        if source==None:
              ...
              x = F.relu(x)
              x = torch.cat([x, self.stop_action], dim=0)
              
              out = torch.matmul(x[0], self.link_pred_source).matmul(x[0:].transpose(1, 0))
              ...
              indices = [i for i, x in enumerate(edge_index[0]) if x == source]
              filter_source = list(pd.Series(edge_index[1])[indices])
              out = torch.matmul(x[source], self.link_pred_target).matmul(x[filter_source].transpose(1, 0))
              
              return F.softmax(out, dim=0)
              
    def value_function(self, x, edge_index, source=None):
          if source==None:
              x = self.conv1(x, edge_index)
              x = F.relu(x)
              x = self.conv2(x, edge_index)
              ...
              x = torch.cat((x[0], torch.zeros(x[0].shape)), dim=0)
              out = torch.matmul(x, self.output)
              ...
              return out
               ...
               x = F.relu(x)
               x = torch.cat((x[0], x[source]), dim=0)
...
'''               
Code below adapted from https://github.com/RexYing/gnn-model-explainer
